...
| [`rjv-jev-decisions`](./skills/rjv-jev-decisions/SKILL.md) | Hand closed-set decisions in a Claude Code or Codex session to Jev, TypeSafe's System One model, for about a second and a fraction of a cent each. Main use: pick the model tier for a delegated task (haiku/sonnet/opus, gpt-5.4-mini/gpt-5.5 effort, or local Ollama), with confidence and stakes gates applied in code. Also a keep-or-escalate check on a cheap worker's report, and a Build mode second opinion for `rjv-work-plan`. curl + jq scripts, exit 3 means fall back, never block. |
